...
namespace Step55
{
                                        ...
                                        Vector<double> &  values) const
  {
    ...
    const int k = 3;
    values[0] = -4*pi2*alpha*(1+beta*sin(2*k*pi*p[0])*cos(2*k*pi*p[1]))*sin(2*pi*R_y)*(2*cos(2*pi*R_x)-1)+4*pow(pi,2)*sin(2*pi*R_x);
    
    values[1] = -4*pi2*alpha*(1+beta*sin(2*k*pi*p[0])*cos(2*k*pi*p[1]))*sin(2*pi*R_x)*(2*cos(2*pi*R_y)-1)+4*pow(pi,2)*sin(2*pi*R_y);
                  ...
                  for (unsigned int j = 0; j < dofs_per_cell; ++j)
                    {
                      cell_matrix(i, j) +=
                        ...
                        fe_values.JxW(q);
                      cell_matrix2(i, j) += 1.0 / viscosity_values[q] * phi_p[i] *
